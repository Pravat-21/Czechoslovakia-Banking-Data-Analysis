...
SELECT * FROM PP_LOAN;--682
SELECT * FROM PP_ACOUNT;--4500
SELECT * FROM PP_TRNX WHERE PURPOSE='Loan Payment';--13579
SELECT * FROM PP_DISTRICT;--77
SELECT * FROM PP_CLIENT;--5369K
--------------------------------------------------------------------------------------------
CREATE OR REPLACE TABLE LOAN_MATRICS_DETAILS AS
(SELECT
    ...
    DISTRICT_NAME,
    BANK_NAME,
    COUNT(CASE WHEN ACCOUNT_TYPE='Salary account' THEN LOAN_ID ELSE NULL END)AS LOAN_FM_SLRY_ACC,
    ROUND(AVG(CASE WHEN ACCOUNT_TYPE='Salary account' THEN LOAN_AMOUNT ELSE 0 END),2) AS AVG_AMT_SLRY_LOAN,
    COUNT(CASE WHEN ACCOUNT_TYPE='Savings account' THEN LOAN_ID ELSE NULL END)AS LOAN_FM_SVING_ACC,
    ROUND(AVG(CASE WHEN ACCOUNT_TYPE='Savings account' THEN LOAN_AMOUNT ELSE 0 END),2) AS AVG_AMT_SVNG_LOAN,
    COUNT(CASE WHEN ACCOUNT_TYPE='NRI account' THEN LOAN_ID ELSE NULL END)AS LOAN_FM_NRI_ACC,
    ROUND(AVG(CASE WHEN ACCOUNT_TYPE='NRI account' THEN LOAN_AMOUNT ELSE 0 END),2) AS AVG_AMT_NRI_LOAN,
    COUNT(CASE WHEN LOAN_STATUS='Client in debt' THEN LOAN_ID ELSE NULL END)AS CUNT_DEBT_LOAN,
    COUNT(CASE WHEN LOAN_STATUS='Running Contract' THEN LOAN_ID ELSE NULL END)AS CUNT_RUNNING_CNTRT_LOAN,
    COUNT(CASE WHEN LOAN_STATUS='Contract Finished' THEN LOAN_ID ELSE NULL END)AS CUNT_FINISH_LOAN,
    COUNT(CASE WHEN LOAN_STATUS='Loan not payed' THEN LOAN_ID ELSE NULL END)AS CUNT_UNPAID_LOAN
FROM
(SELECT
    AC.ACCOUNT_ID,L.LOAN_ID,A.TRANS_ID, AC.DISTRICT_ID,AC.ACCOUNT_TYPE,L.LOAN_DATE,
    L.AMOUNT AS LOAN_AMOUNT,L.DURATION,L.PAYMENTS,
    L.STATUS AS LOAN_STATUS,A.DATE AS TRNX_DATE,A.BALANCE,A.BANK AS BANK_NAME,D.DISTRICT_NAME,D.REGION,D.NO_OF_CITIES,
    D.AVERAGE_SALARY,D.NO_OF_ENTREPRENEURS_PER_1000_INHABITANTS,D.NO_COMMITTED_CRIME_2017,D.NO_COMMITTED_CRIME_2018
FROM 
(SELECT * FROM PP_TRNX WHERE PURPOSE='Loan Payment')A
LEFT OUTER JOIN PP_ACOUNT AC ON A.ACCOUNT_ID=AC.ACCOUNT_ID
LEFT OUTER JOIN PP_LOAN L ON AC.ACCOUNT_ID=L.ACCOUNT_ID
LEFT OUTER JOIN PP_DISTRICT D ON AC.DISTRICT_ID=D.DISTRICT_CODE)
GROUP BY 1,2,3,4,5
ORDER BY 1 DESC,3,4,5);

------------------------------------------------------------------------------------------------------------------------
SELECT * FROM PROJECT1.PUBLIC.CREDIT_CARD_DETAILS_MATIX;
SELECT * FROM PROJECT1.PUBLIC.ACCOUNT_ANALYSIS_DETAILS;
SELECT * FROM PROJECT1.PUBLIC.LOAN_MATRICS_DETAILS;

SELECT * FROM PP_LOAN;
SELECT COUNT(LOAN_ID) FROM PP_LOAN WHERE STATUS='Client in debt';
SELECT SUM(CUNT_DEBT_LOAN) FROM LOAN_MATRICS_DETAILS;

SELECT * FROM PP_TRNX;--1.0M
SELECT * FROM PP_LOAN;--682 LOAN_ID
SELECT * FROM PP_ACOUNT;--4.5K ACCOUNT_ID
SELECT * FROM PP_CARD;--892 CARD ID
SELECT * FROM PP_DISP;--5.4K

CREATE OR REPLACE TABLE LOAN_CLIENT_CARD_TRNX AS 
(SELECT  CL.CLIENT_ID ,A.ACCOUNT_ID,A.DISTRICT_ID,C.CARD_ID,C.DISP_ID,L.LOAN_ID,T.TRANS_ID,A.FREQUENCY,A.DATE AS ACCOUNT_DATE,
A.ACCOUNT_TYPE,C.TYPE,C.ISSUED AS CARD_ISSUED_DATE,L.LOAN_DATE,L.AMOUNT AS LOAN_AMOUNT,L.DURATION AS LOAN_DURATION,L.PAYMENTS AS LOAN_PAYMENT,L.STATUS AS LOAN_STATUS,T.DATE AS TRANS_DATE,T.AMOUNT AS TRANS_AMOUNT,T.BALANCE,T.BANK AS BANK_NAME,T.ACCOUNT_PARTNER,CL.SEX,CL.AGE

FROM 
PP_ACOUNT A LEFT OUTER JOIN PP_DISP D ON A.ACCOUNT_ID=D.ACCOUNT_ID
LEFT OUTER JOIN PP_CARD C ON D.DISP_ID=C.DISP_ID
LEFT OUTER JOIN PP_LOAN L ON A.ACCOUNT_ID=L.ACCOUNT_ID 
LEFT OUTER JOIN PP_TRNX T ON A.ACCOUNT_ID=T.ACCOUNT_ID
LEFT OUTER JOIN PP_CLIENT CL ON D.CLIENT_ID=CL.CLIENT_ID
WHERE T.PURPOSE='Loan Payment');
---------------------------------------------------------------------------------------

SELECT * FROM PROJECT1.PUBLIC.LOAN_CLIENT_CARD_TRNX;
UPDATE PROJECT1.PUBLIC.LOAN_CLIENT_CARD_TRNX
SET TYPE='Not a Card user' WHERE TYPE IS NULL;
...
LEFT OUTER JOIN PP_LOAN L ON A.ACCOUNT_ID=L.ACCOUNT_ID 
LEFT OUTER JOIN PP_TRNX T ON A.ACCOUNT_ID=T.ACCOUNT_ID
LEFT OUTER JOIN PP_CLIENT CL ON D.CLIENT_ID=CL.CLIENT_ID)A
GROUP BY 1;

SELECT * FROM PP_CLIENT;
-----------------------------------------------------------------------------------------------

CREATE OR REPLACE PROCEDURE AGE_UPDATE()
RETURNS STRING
LANGUAGE SQL
AS
$$
...
CREATE OR REPLACE PROCEDURE BANK_TRNX_LOAN_KPI()
RETURNS STRING
LANGUAGE SQL
AS
$$
...
-------------------------------------------------------------------------------------------
--TASK SETUP
CREATE OR REPLACE TASK AGE_UPDATE_KPI
WAREHOUSE=PRAVAT_SQL_1
SCHEDULE='USING CRON 30 5 * * 6 UTC'
--SCHEDULE='2 MINUTES'
AS CALL AGE_UPDATE();

CREATE OR REPLACE TASK BANK_TRNX_LOAN_KPI
WAREHOUSE=PRAVAT_SQL_1
SCHEDULE='USING CRON 30 6 * * 6 UTC'
--SCHEDULE='4 MINUTES'
AS CALL BANK_TRNX_LOAN_KPI();

SHOW TASKS;
ALTER TASK AGE_UPDATE_KPI RESUME;
ALTER TASK BANK_TRNX_LOAN_KPI RESUME;

ALTER TASK  AGE_UPDATE_KPI SUSPEND;
ALTER TASK  BANK_TRNX_LOAN_KPI SUSPEND;
